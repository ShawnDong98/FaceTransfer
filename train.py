...
class AE():
    def __init__(self, config, src_dataloader, dst_dataloader, device='cpu'):
        ...
        self.e_optimizer = torch.optim.Adam(
            self.E.parameters(), 0.0001, [0.0, 0.9])
        self.g_src_optimizer = torch.optim.Adam(
            self.G_src.parameters(), 0.0001, [0.0, 0.9])
        self.d_src_optimizer = torch.optim.Adam(
            self.D_src.parameters(), 0.0004, [0.0, 0.9])

        self.g_dst_optimizer = torch.optim.Adam(
            self.G_dst.parameters(), 0.0001, [0.0, 0.9])
        self.d_dst_optimizer = torch.optim.Adam(
            self.D_dst.parameters(), 0.0004, [0.0, 0.9])
    ...
    def train(self):
        ...
        for step in range(self.state['iter'], self.config.total_iter):
            ...
            rec_src_loss = Rec_loss(
                self.D_src, target_img, rec_img).contiguous()
            ...
            # train Generator
            g_loss = G_loss(self.D_src, self.G_src, rec_img,
            ...
            # train Discriminator
            d_src_loss = D_loss(self.D_src, self.G_src,
                                target_img, rec_img, self.latent_dim)
            ...
            # train Generator
            g_loss = G_loss(self.D_dst, self.G_dst, rec_img,
                            self.batch_size, self.latent_dim, self.device)
            ...
            # train Discriminator
            d_dst_loss = D_loss(self.D_dst, self.G_dst,
                                target_img, rec_img, self.latent_dim)
    ...
    def inference(self):
        ...
        src_target = src_target.squeeze().detach(
        ).cpu().numpy().transpose([1, 2, 0])*0.5 + 0.5
        rec_src = rec_src.squeeze().detach().cpu(
        ).numpy().transpose([1, 2, 0])*0.5 + 0.5
        ...
        dst_target = dst_target.squeeze().detach(
        ).cpu().numpy().transpose([1, 2, 0])*0.5 + 0.5
        rec_dst = rec_dst.squeeze().detach().cpu(
        ).numpy().transpose([1, 2, 0])*0.5 + 0.5
